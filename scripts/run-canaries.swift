...
try expectAllowed(#"workdir=/tmp/codex-preview-123; rm -rf "$workdir""#)
try expectAllowed(#"rm -f "$generated_file""#)
...
try expectDenied("rm -rf $HOME", rule: "filesystem.rm-broad")
try expectDenied(#"rm -rf "${HOME:?}""#, rule: "filesystem.rm-dynamic-target")
try expectDenied(#"target=$HOME; rm -rf "$target""#, rule: "filesystem.rm-broad")
try expectDenied(#"rm -rf "/Users/$USER""#, rule: "filesystem.rm-dynamic-target")
try expectDenied(#"false && workdir=/tmp/codex-preview-123; rm -rf "$workdir""#, rule: "filesystem.rm-dynamic-target")
try expectDenied("printf safe | git reset --hard", rule: "git.reset-hard")
try expectDenied(#"printf "$(whoami)""#, rule: "parser.ambiguous-shell")
try expectDenied("git restore ./", rule: "git.restore-worktree")
try expectDenied("git push --force-with-lease=main origin main", rule: "git.force-protected")
try expectDenied("docker --context prod system prune --volumes", rule: "docker.prune-volumes")
try expectDenied("terraform -chdir=production destroy", rule: "terraform.destroy")
try expectDenied("aws --profile prod s3 rm s3://bucket --recursive", rule: "cloud.recursive-delete")
try expectDenied("diskutil quiet eraseDisk APFS Empty /dev/disk99", rule: "device.diskutil-erase")
try expectDenied("rsync -av ./empty/ /Users/example/ --delete", rule: "filesystem.sync-delete")
